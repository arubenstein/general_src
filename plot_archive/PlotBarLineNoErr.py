...
def plot_line ( y, ax,col):
#    color = ax._get_lines.color_cycle.next()
    line=ax.axhline(y, color=col,linewidth=3.0)
    return line
...
def main(args):
    ...
    for ind in range(0, len(lines)-1,series_l+1):
        print lines[ind]
    ...
    rects.append(plot_line(const_bits,ax,color[-1]))
    label.append("Expt")

    prop = fm.FontProperties(fname="/Users/arubenstein/.matplotlib/mpl-data/fonts/ttf/Helvetica.ttf")
 
    lgd = ax.legend( rects, label,loc='upper center',ncol=4, fancybox=True,bbox_to_anchor=(0., 1.07, 1., .105),prop={'size':10})
