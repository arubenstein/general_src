...
#	filename = p.rsplit('.',1)[0] + ".clean.pdb"
	...
	num_res_start = 1
	num_res_end = chains[idx_pept].total_residue()
	...
	print newpose.total_residue()
	print pose.total_residue()

	if newpose.total_residue() < pose.total_residue() :
	    newpose.append_residue_by_jump(pose.residue(newpose.total_residue()+1), newpose.total_residue(), "", "", 1)
            newpose.pdb_info().number(newpose.total_residue(),1)
            newpose.pdb_info().chain(newpose.total_residue(),'X')
	    for ind, i in zip( range( 2, pose.total_residue() - newpose.total_residue() + 2 ), range(newpose.total_residue()+1, pose.total_residue()+1) ):
                newpose.append_polymer_residue_after_seqpos(pose.residue(i), newpose.total_residue(),0)
                newpose.pdb_info().number(newpose.total_residue(),ind)
                newpose.pdb_info().chain(newpose.total_residue(),'X')
