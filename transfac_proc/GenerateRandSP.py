...
def main(args):
    ...
    outfile_cosine = outfile + "_cosine.txt"
    outfile_frob = outfile + "_frob.txt"
    outfile_aad = outfile + "_aad.txt"
    outfile_jsd = outfile + "_jsd.txt"
    outfile_auc = outfile + "_auc.txt"

    dist_cosine = open(outfile_cosine,"w")
    dist_frob = open(outfile_frob,"w")
    dist_aad = open(outfile_aad,"w")
    dist_jsd = open(outfile_jsd,"w")
    dist_auc = open(outfile_auc,"w")

    for i in range(1,100001):
	freq_shuff = shuffleSP( list_freq,n_pos )
        nda_freq_shuff = numpy.array( [ freq_shuff] )
        nda_freq_comp = numpy.array( [ freq_comp] )
        flat_freq_shuff = numpy.ndarray.flatten( nda_freq_shuff )
        flat_freq_comp = numpy.ndarray.flatten( nda_freq_comp )
        jsd1 = [ JSDivergence ( i, g ) for i,g in zip( freq_shuff, freq_comp )]
        f = [ JSDivergence ( i, g ) for i,g in zip( freq_shuff, freq_comp )]
        a = [ JSDivergence ( i, g ) for i,g in zip( freq_shuff, freq_comp )]
        jsd1 = [ JSDivergence ( i, g ) for i,g in zip( freq_shuff, freq_comp )]
        auc = [ areaUnderCurve ( i, g ) for i, g in zip( freq_comp, freq_in )]
